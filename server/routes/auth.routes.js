...
router.post('/login', async (req, res) => {
    ...
    const token = jwt.sign({id: existingUser.dataValues.id, role: existingUser.dataValues.password}, process.env.TOKEN_SECRET, {
        expiresIn: '1h'
    })

    res.status(200).json({success: true, message: 'User successfully authenticated', data: token})
})
